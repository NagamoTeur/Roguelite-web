...
class Salle {
  constructor(x, y) {
    ...
    this.y = y;
    this.voisins = [];
    this.contenu = []; // À remplir avec les éléments de la salle
  }
}

class Joueur {
  constructor(x, y, salle) {
    this.x = x;
    this.y = y;
...
function genererSalleInitiale() {
  const salleInitiale = new Salle(0, 0);
  joueur = new Joueur(salleInitiale.x, salleInitiale.y, salleInitiale);
  return salleInitiale;
}

function explorer(salle) {
  // ... (Logique pour vérifier si le joueur peut se déplacer dans une direction)
  if (peutSeDeplacer(direction)) {
    // Créer une nouvelle salle dans cette direction si elle n'existe pas
    const nouvelleSalle = creerSalleVoisine(salle, direction);
    // Ajouter un couloir entre les deux salles
